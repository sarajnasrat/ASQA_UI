...
export const CertificationRequestReport = () => {
  ...
  const currentShamsiDate = new DateObject({
    date: currentDate,
    calendar: persian,
  });
  const currentShamsiYear = Number(currentShamsiDate.year);
  const currentShamsiMonth = Number(currentShamsiDate.month.number);
  ...
  const [year, setYear] = useState<number>(currentShamsiYear);
  const [month, setMonth] = useState<number>(currentShamsiMonth);
  ...
  const toShamsiRange = (selectedYear: number, selectedMonth?: number) => {
    ...
    if (!selectedMonth && end.isLeap) {
      end.day = 30;
    }

    const startGregorian = start.toDate();
    const endGregorian = end.toDate();

    const startDateTime = new Date(startGregorian);
    startDateTime.setHours(0, 0, 0, 0);

    const endDateTime = new Date(endGregorian);
  ...
  const exportToExcel = () => {
    ...
    try {
      const excelData = [
        ...
        {
          Category: t("report.requestStatistics"),
          Metric: t("report.totalRequests"),
          Value: reportData.totalRequests || 0,
        },
        {
          Category: t("report.requestStatistics"),
          Metric: t("report.submitted"),
          Value: reportData.totalSubmittedRequest || 0,
        },
        {
          Category: t("report.requestStatistics"),
          Metric: t("report.inProgress"),
          Value: reportData.inProgressRequest || 0,
        },
        {
          Category: t("report.requestStatistics"),
          Metric: t("report.completed"),
          Value: reportData.totalCompletedRequest || 0,
        },
        {
          Category: t("report.requestStatistics"),
          Metric: t("report.rejected"),
          Value: reportData.totalRejectedRequest || 0,
        },
        { Category: "", Metric: "", Value: "" },
        {
          Category: t("report.userStatistics"),
          Metric: t("report.totalUsers"),
          Value: reportData.totalUsers || 0,
        },
        {
          Category: t("report.userStatistics"),
          Metric: t("report.activeUsers"),
          Value: reportData.activeUsers || 0,
        },
        {
          Category: t("report.userStatistics"),
          Metric: t("report.inactiveUsers"),
          Value: reportData.inActiveUsers || 0,
        },
        { Category: "", Metric: "", Value: "" },
        {
          Category: t("report.committeeStatistics"),
          Metric: t("report.totalCommittee"),
          Value: reportData.totalCommitee || 0,
        },
        {
          Category: t("report.committeeStatistics"),
          Metric: t("report.activeCommittee"),
          Value: reportData.activeCommitee || 0,
        },
        {
          Category: t("report.committeeStatistics"),
          Metric: t("report.inactiveCommittee"),
          Value: reportData.inActiveCommitee || 0,
        },
        { Category: "", Metric: "", Value: "" },
        {
          Category: t("report.additionalStatistics"),
          Metric: t("report.totalCompanies"),
          Value: reportData.totalCompanies || 0,
        },
        {
          Category: t("report.additionalStatistics"),
          Metric: t("report.totalCertifications"),
          Value: reportData.totalCertifications || 0,
        },
        {
          Category: t("report.additionalStatistics"),
          Metric: t("report.totalMenuItems"),
          Value: reportData.totalMenu || 0,
        },
      ];
  ...
  const exportDetailedToExcel = async () => {
    ...
    try {
      ...
      reqSectionRow.getCell(1).alignment = {
        horizontal: "center",
        vertical: "middle",
      };

      worksheet.addRow([
        t("report.totalRequests"),
        reportData.totalRequests || 0,
      ]);
      worksheet.addRow([
        t("report.submitted"),
        reportData.totalSubmittedRequest || 0,
      ]);
      worksheet.addRow([
        t("report.inProgress"),
        reportData.inProgressRequest || 0,
      ]);
      worksheet.addRow([
        t("report.completed"),
        reportData.totalCompletedRequest || 0,
      ]);
      worksheet.addRow([
        t("report.rejected"),
        reportData.totalRejectedRequest || 0,
      ]);
      ...
      worksheet.mergeCells(
        `A${userSectionRow.number}:B${userSectionRow.number}`,
      );
      userSectionRow.getCell(1).font = {
        bold: true,
        size: 12,
        color: { argb: "FFFFFFFF" },
      };
      ...
      userSectionRow.getCell(1).alignment = {
        horizontal: "center",
        vertical: "middle",
      };
      ...
      worksheet.addRow([
        t("report.inactiveUsers"),
        reportData.inActiveUsers || 0,
      ]);

      // Committee Statistics Section Header - Merged across both columns
      const committeeSectionRow = worksheet.addRow([
        t("report.committeeStatistics"),
        "",
      ]);
      worksheet.mergeCells(
        `A${committeeSectionRow.number}:B${committeeSectionRow.number}`,
      );
      committeeSectionRow.getCell(1).font = {
        bold: true,
        size: 12,
        color: { argb: "FFFFFFFF" },
      };
      ...
      committeeSectionRow.getCell(1).alignment = {
        horizontal: "center",
        vertical: "middle",
      };

      worksheet.addRow([
        t("report.totalCommittee"),
        reportData.totalCommitee || 0,
      ]);
      worksheet.addRow([
        t("report.activeCommittee"),
        reportData.activeCommitee || 0,
      ]);
      worksheet.addRow([
        t("report.inactiveCommittee"),
        reportData.inActiveCommitee || 0,
      ]);

      // Additional Statistics Section Header - Merged across both columns
      const additionalSectionRow = worksheet.addRow([
        t("report.additionalStatistics"),
        "",
      ]);
      worksheet.mergeCells(
        `A${additionalSectionRow.number}:B${additionalSectionRow.number}`,
      );
      additionalSectionRow.getCell(1).font = {
        bold: true,
        size: 12,
        color: { argb: "FFFFFFFF" },
      };
      ...
      additionalSectionRow.getCell(1).alignment = {
        horizontal: "center",
        vertical: "middle",
      };

      worksheet.addRow([
        t("report.totalCompanies"),
        reportData.totalCompanies || 0,
      ]);
      worksheet.addRow([
        t("report.totalCertifications"),
        reportData.totalCertifications || 0,
      ]);
        ...
        row.eachCell((cell: any, colNumber: number) => {
          ...
          if (colNumber === 1 && rowNumber > 4) {
            ...
            const isSectionHeader =
              cell.fill &&
              cell.fill.fgColor &&
              cell.fill.fgColor.argb === "FF4472C4";
            if (!isSectionHeader) {
              cell.font = { bold: true };
            }
          }
      ...
      const metricsRows = [
        {
          type: t("report.requestStatistics"),
          name: t("report.totalRequests"),
          value: reportData.totalRequests || 0,
        },
        {
          type: t("report.requestStatistics"),
          name: t("report.submitted"),
          value: reportData.totalSubmittedRequest || 0,
        },
        {
          type: t("report.requestStatistics"),
          name: t("report.inProgress"),
          value: reportData.inProgressRequest || 0,
        },
        {
          type: t("report.requestStatistics"),
        ...
        {
          type: t("report.requestStatistics"),
          name: t("report.rejected"),
          value: reportData.totalRejectedRequest || 0,
        },
        {
          type: t("report.userStatistics"),
          name: t("report.totalUsers"),
          value: reportData.totalUsers || 0,
        },
        {
          type: t("report.userStatistics"),
          name: t("report.activeUsers"),
          value: reportData.activeUsers || 0,
        },
        {
          type: t("report.userStatistics"),
          name: t("report.inactiveUsers"),
          value: reportData.inActiveUsers || 0,
        },
        {
          type: t("report.committeeStatistics"),
          name: t("report.totalCommittee"),
          value: reportData.totalCommitee || 0,
        },
        {
          type: t("report.committeeStatistics"),
          name: t("report.activeCommittee"),
          value: reportData.activeCommitee || 0,
        },
        {
          type: t("report.committeeStatistics"),
          name: t("report.inactiveCommittee"),
          value: reportData.inActiveCommitee || 0,
        },
        {
          type: t("report.additionalStatistics"),
          name: t("report.totalCompanies"),
          value: reportData.totalCompanies || 0,
        },
        {
          type: t("report.additionalStatistics"),
          name: t("report.totalCertifications"),
          value: reportData.totalCertifications || 0,
        },
        {
          type: t("report.additionalStatistics"),
          name: t("report.totalMenuItems"),
          value: reportData.totalMenu || 0,
        },
      ];
  ...
  const getMonthName = (monthNumber: number) => {
    return shamsiMonthNames[monthNumber - 1] || String(monthNumber);
  };

  const StatCard = ({ title, value, icon }: any) => (
    <div className="group rounded bg-white p-2 shadow-sm transition-all duration-300 hover:shadow-md">
      <div className="flex items-center justify-between">
        <div className="flex-1">
          <div className="flex justify-between">
            <p className="text-sm font-medium text-gray-600">{title}</p>
            <div className="rounded-full bg-gray-100 p-1 transition-transform duration-300 group-hover:scale-110">
              {icon}
            </div>
          </div>

          <p className="mt-1 font-bold text-gray-900">{value || 0}</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="border rounded-2xl border-gray-200 bg-white p-4 shadow-sm sm:p-6 pl-5 pr-5 max-w-8xl mx-auto mt-3">
      {/* Header Section */}
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">
            {t("report.dashboardTitle")}
          </h2>
        ...
        <div className="flex flex-wrap items-end gap-4">
            ...
            <>
              <div className="min-w-45 flex-1">
                ...
                <SmartDatePicker
                  value={startDate}
                  onChange={(date) =>
                    handleStartDateChange(toIsoDateString(date))
                  }
              ...
              <div className="min-w-45 flex-1">
                ...
                <SmartDatePicker
                  value={endDate}
                  onChange={(date) =>
                    handleEndDateChange(toIsoDateString(date))
                  }
                  calendarType="persian"
                />
              </div>
          ...
          <div className="min-w-50 flex-1">
            ...
            <div className="relative">
              ...
              >
                ...
                <svg
                  className={`h-4 w-4 transition-transform ${showModuleDropdown ? "rotate-180" : ""}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              {showModuleDropdown && (
                <div className="absolute left-0 right-0 z-10 mt-2 max-h-64 overflow-y-auto rounded-xl border border-gray-200 bg-white shadow-lg">
                  <div className="p-2">
                    <button
                      onClick={() =>
                        handleModulesChange(Object.values(ReportModule))
                      }
          ...
          <div className="w-full sm:w-auto">
            <button
              ...
              disabled={
                loading || (reportType === "range" && (!startDate || !endDate))
              }
      ...
      <div className="rounded-2xl bg-gray-50 p-4 sm:p-5">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">
              {t("report.reportResults")}
            </h3>
            {reportData && (
              <p className="mt-1 text-xs text-gray-500">
                {t("report.lastUpdated")}:{" "}
                {IslamicDateFormatter.formatQamari(new Date(), true)}
              </p>
            )}
          </div>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            <div>
              {/* <h4 className="mb-3 flex items-center gap-1 text-sm font-semibold text-gray-700">
