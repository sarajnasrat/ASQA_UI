...
interface DistrictCreateProps {
  provinces: Province[];
  onClose: () => void;
  onSuccess: () => void;
}

export const DistrictCreate: React.FC<DistrictCreateProps> = ({
  provinces,
  onClose,
  onSuccess,
}) => {
  ...
  const onSubmitForm = async (data: DistrictFormValues) => {
    ...
    try {
      ...
      onSuccess();
      reset(); // reset form after success if needed
    } catch (error: any) {
      toast.current?.show({
        severity: "error",
        summary: t("common.error"),
        detail: error.response?.data?.message || t("district.createFailed"),
        life: 5000,
      });
    } finally {
        ...
        >
          ...
          >
            ...
            <div>
              ...
              />
              {errors.provinceId && (
                <p className="text-sm text-red-600 mt-1">{errors.provinceId.message}</p>
              )}
            </div>
