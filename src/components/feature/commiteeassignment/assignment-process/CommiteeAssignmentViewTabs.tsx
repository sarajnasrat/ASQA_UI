...
import { Building2, File, FileText } from "lucide-react";
...
const CommiteeAssignmentViewTabs: React.FC<Props> = ({
  activeTab,
  setActiveTab,
  totalDocuments,
  t,
      ...
      <div className="flex flex-wrap gap-4 md:gap-6">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() =>
              setActiveTab(tab.id as "details" | "company" | "documents")
            }
          ...
          >
            ...
            <span className="sm:hidden">
              {tab.id === "details"
                ? t("commitee.assignment.tabs.detailsShort")
                : tab.id === "company"
                  ? t("commitee.assignment.tabs.companyShort")
                  : t("commitee.assignment.tabs.documentsShort")}
            </span>
            {tab.badge ? (
...
export default CommiteeAssignmentViewTabs;
